...
class MY_GUI():
    ...
    def set_init_window(self):
        ...
        self.file = Button(labelframe, text="添加破解文件目录", command=self.add_mm_file).grid(column=2, row=1)
        ...
        while True:
            try:
                ...
                if self.bool1:
                    # print("密码正确："+pwdStr
                    # res = "密码:%s 正确 \n"%self.pwdStr;
                    self.res = "===正确===  wifi名:%s  破解密码为：%s " % (self.get_wifissid, self.pwdStr)
